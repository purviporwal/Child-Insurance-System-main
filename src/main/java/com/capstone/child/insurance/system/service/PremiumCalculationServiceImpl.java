...
//		Optional<PremiumCalculation> p1 = premCalcRepository.findById(id);
	...
	private double IncomeFactor(double annual_income) {
		double income = 0;
		double income_factor = 0.1 * annual_income;
		income = income_factor / 12;
	...
	private double amountByGender(Gender Genderofchild, Integer id) {
		...
		if (childOpt.isPresent()) {
			child = childOpt.get();
			if (child.getChildGender() == Gender.Male) {
	...
	@Override
	public Double addPremCalc(PremiumCalculation newPremCalc) {
